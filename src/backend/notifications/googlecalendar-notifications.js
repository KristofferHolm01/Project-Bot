...
oAuth2Client.setCredentials({
  ...
  try {
    ...
    if (webhooks.length > 0) {
      removeGoogleWebhook(guildId);
    }

    editSyncTokenGoogleCalendar(integrationId, null); 
...
async function getGoogleEvents(guildId) {
  ...
  try {
    ...
    // Edit the sync token
    editSyncTokenGoogleCalendar(integrationId, events.data.nextSyncToken); 
    ...
    googleNotifications.forEach(notification => {
      if (notification.codename === type && enabledNotifications.some(n => n.name === notification.name)){
        ...
        sendGoogleCalendarToDiscord(element, eventType, discordChannel);
        return;
      }
    });
  }
 ...
 */
function sendGoogleCalendarToDiscord(element, type, discordChannel) {
    ...
    try {
      ...
      sendEmbedToDiscord(discordChannel, embed);   //for testing purposes where it is sent to the test channel
    } catch (error) {
